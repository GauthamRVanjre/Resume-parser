...
/** One-shot guest analysis — uploads PDF + job description, returns result without saving to DB. */
export async function analyzeGuest(file: File, jobDescription: string, guestId: string): Promise<AnalysisResult> {
  const form = new FormData();
  form.append("file", file);
